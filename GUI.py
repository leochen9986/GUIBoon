...
class GUI:
    def __init__(self, window, window_title, video_source="media/1.mp4"):
        ...
        self.volume_label = tk.Label(self.window, textvariable=self.volume_var, width=10 ,font=("Arial", 15),bg="white")
        self.volume_label.update_idletasks()
        self.volume_label.place(x=100)
        self.volume_label.place_forget()
        
        
        self.tds_var = tk.StringVar()
        self.tds_var.set("TDS:0")
        self.tds_label = tk.Label(self.window, textvariable=self.tds_var, width=10 ,font=("Arial", 15),bg="white")
        self.tds_label.update_idletasks()
        self.tds_label.place(y=20)
    ...
    def update(self):
        ...
        if self.ret:
            ...
            #Screen 1a
            if self.esp32_data["Motion_detected"]==1 or self.state=="bottleplaced":
                ...
                #Screen 2            
                elif (self.esp32_data["Bottle_placed"]==1 or self.state=="bottleplaced") and not self.state=="bottleremoved":
                    ...
                    self.volume_label.place(x=100)
                    self.tds_label.place(x=100,y=20)
                    
                    

                    if self.esp32_data["Bottle_placed"]==1:
                        self.off_bottletimer=time.time()
                    else:
